...
class FindRegion(Struc):
    def __init__(self,vectors,channelMap={'micropore':{'D'}, 'mesopore':{'F'}, 'surface':{'H'}}):
        self.boxLengths = vectors
        #self.gridName = '/Users/dejacor/Documents/butanol-water/323K/MFI/energy_grid_114_O.out'
        self.mergedGridName = '/home/siepmann/josep180/Work/OFS/SPP/SPPgridcheck7.xyz'
        self.gridName = self.mergedGridName #'/home/siepmann/josep180/Work/BTX/energy_grid_nano.out'
    ...
    def initGrids(self):
        ...
        #with open(self.gridName) as g:
            #line = next(g)
        self.numBins = tuple([600, 600, 1])
            #self.numBins = tuple(map(int, line.split()))
        #print('self.numBins = ', self.numBins)
...
class Channel(FindRegion):
    ...
    def __init__(self, regionMap):
        ...
        my_parser.parser.add_argument('-abc','--vectors', help='unit cell vectors. For folding coordinates into a unit cell',
                                      type = float, nargs = '+', default = [60.066, 59.697,13.383])
        my_parser.parser.add_argument('-g','--gridName', help='energy grid file name',
                        default = '/home/siepmann/josep180/Work/BTX/energy_grid_nano.out', type=str)
        my_parser.parser.add_argument('-M', '--mergedGridName', help='energy grid merged file name',
                        default = '/home/siepmann/josep180/Work/OFS/SPP/SPPgridcheck7.xyz',type=str)
    ...
    def avgIndep(self):
        ...
        for mol in self.N.keys():
            ...
            for channel, values in self.N[mol].items():
                ...
                #self.averages[self.feed][mol][channel]['raw'][i] = values[i]
                if isinstance(values, np.ndarray):
                    self.averages[self.feed][mol][channel]['raw'] = values.tolist()
                else:
...
#   mapToExp = {'straight':{'H','Li'}, 'zig-zag':{'Be','B','He','D'}}
    mapToExp = {'micropore':{'D'}, 'mesopore':{'F'}, 'surface':{'H'}}
    M = Channel(mapToExp)
    M.main()
